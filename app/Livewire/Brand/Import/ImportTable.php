...
final class ImportTable extends PowerGridComponent
{
    ...
    public function datasource(): Builder
    {
        ...
        return ImportLog::query()->where('model_name', config('constants.import_type.brand'));
    }
        ...
        if ($row->status == config('constants.import_csv_log.status.key.fail')) {
            return [
                ...
                Button::add('view')
                    ...
                    ->dispatchTo('brand.import.import-error-page', 'viewImportErrors', ['errorLogs' => $row->error_log]),
            ];
        }
