@if (count($enabledFilters))
    ...
    >
        @if (count($enabledFilters) > 1)
            ...
            >
                <span
                    class="badge rounded-pill bg-danger text-white">{{ trans('livewire-powergrid::datatable.buttons.clear_all_filters') }}
                    <svg
                        width="10"
                        stroke="currentColor"
                        fill="none"
        ...
        @foreach ($enabledFilters as $filter)
            @isset($filter['label'])
                <div
                    ...
                    style="cursor: pointer; padding-right: 4px"
                >
                    <span class="badge rounded-pill bg-primary text-white">{{ $filter['label'] }} &nbsp;
                        <svg
                            width="10"
                            stroke="currentColor"
                            fill="none"
